...
def get_heatmap_layer(layer_id: str):
    ...
    Supported: crime, crime_ncrb, garbage_dump, traffic, pothole, drainage,
               stp, street_dogs, crashes, tax_collection, weather_station, bescom
    """
    category_map = {
    ...
    return [GridPoint(lat=p["lat"], lng=p["lng"], intensity=p["intensity"], label=p.get("label")) for p in points]
